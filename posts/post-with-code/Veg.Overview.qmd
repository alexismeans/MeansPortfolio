...
quality <- read_excel(q, "Sheet 1")
plant <- read_excel(db, "PlantList")

plant <- plant %>% 
  rename(FG = FunctionalGroup) %>% 
  select(Spp, Family, Genus, Species, CommonName, FG,  Status) %>% 
  rename(Code = Spp)

quality <- quality %>% 
  mutate(
    Pheno = str_split(Description, "_", simplify = TRUE)[, 3],
    Part = sub("^([^_]*_){3}", "", Description),
    PVT = substr(Description, 1, 3)
  ) %>% 
  rename(DE = `DE_KJ/g`,
         DP = `DP_g100g`)
