...
I was curious to see how green-up timing varied across the different vegetation communities, especially in the riparian areas. The timing in Grassland, Intermediate, and Shrubland communities was fairly similar, while Scabland and Sparse areas progressed the fastest. As expected, the Riparian areas showed a slower progression through green-up compared to the other communities, likely providing more nutritious forage into late June or early July, just as lactation is hopefully slowing for many of the ewes. I think we did a pretty good job at capturing the beginning of the fall green-up as well. I suspect it gets a little greener through November/December than what we were able to capture, but I think this will be enough to get at the inferences we need to make from Fall selection.
...
All results now include quality metrics from both 2024 and 2025. I also added a separate table listing the nutritional metrics to make them easier to reference rather than hunting through the graphs.

The column **"Metric_Source"** indicates whether the nutritional metrics for a given species and phenological stage were directly measured or estimated from a broader grouping. The finest level is **"Descrip_Year"**, which represents a unique combination of Year, PVT, Species Code, Phenology, Plant Part, and Season. If no sample exists for that exact combination, the mean of observations sharing the same PVT, Code, Phenology, Part, and Season is used instead. From there, the process works up through a hierarchy — Code, Genus, Family, and finally Functional Group — until a match is found. Functional Group is the broadest level and applies to only one observation; the vast majority of entries have original measured metrics. A species code key is also included for each section so no guessing is required.
...
library(DT)
db <- "C:/Users/Alexis Means/Documents/Project/Nutrition Sampling/VegDatabases/Working.composition.data.xlsx"
q <- "C:/Users/Alexis Means/Documents/Project/Nutrition Sampling/R code/quality/processed.data/total.qualitydata.xlsx"

quality <- read_excel(q, "Sheet1")
plant <- read_excel(db, "PlantList")

quality <- quality %>% 
  rename(DE = `DE_KJ/g`,
         DP = `DP_g100g`) %>% 
  distinct(CompID, .keep_all = TRUE)
  

grass <- quality %>% 
  filter(FG %in% c("PG", "AG")) %>% 
  distinct(Descrip_Year, .keep_all = TRUE)

# %>%  filter(!CompID %in% c(251, 275, 264))

grass_metrics <- grass %>% 
  select(quality_level, Code, Phenology, Part,  DE, DP, Year, PVT, Season, Status, Descrip_Year, Descrip, Description) %>% 
  rename(Metric_Source = quality_level) %>% 
  mutate(DE = round(DE, 2),
         DP = round(DP, 2))

grass.list <- plant %>% 
  filter(FG_New %in% c("PG", "AG"))
  

datatable(grass.list, options = list(pageLength = 10))
datatable(grass_metrics, options = list(pageLength = 10))
...
I am still trying to figure out why the DE of some of my newly emergent grass species is so low compared to the metrics from 2024. I rejoined my databases and made sure everything was being calculated using the exact same method. I need to dig into this a little further.
...
p <- ggplot(grass, aes(x = DE, y = DP, text = paste(Code, Phenology, sep = "_"))) +
...
#| message: false
forb <- quality %>% 
  filter(FG %in% c("PF", "AF"))%>% filter(!CompID %in% "308") %>% 
  distinct(CompID, .keep_all = TRUE)
...
forb.list <- plant %>% 
  filter(FG_New %in% c("PF", "AF")) 


datatable(forb.list, options = list(pageLength = 10))
datatable(forb_metrics, options = list(pageLength = 10))
...
We maintained the trend after adding the 2025 data that the DE and DP thresholds of various Lomatiums and Balsamroots are typically the highest. These species are everywhere in the spring and typically what we observed the sheep snacking on the most during our hours of observation. It will be really interesting to see which species come out in the metabarcoding results and if this aligns with these trends.

```{r}
#| warning: FALSE
#| message: false

p <- ggplot(forb, aes(x = DE, y = DP, text = paste(Code, Phenology, sep = "_"))) +
  geom_point(aes(color = Status), size = 3) + 
  xlim(11, max(grass$DE) + 0.5) +
  ylim(7, 18) +
  labs(
...
shrub.list <- plant %>% 
  filter(FG_New %in% c("ES", "ET", "DS", "PS"))


datatable(shrub.list, options = list(pageLength = 10))
datatable(shrub_metrics, options = list(pageLength = 10))
...
p <- ggplot(shrub, aes(x = DE, y = DP, text = paste(Code, Phenology, sep = "_"))) +
  geom_point(aes(color = Status), size = 3) + 
...
It was interesting to notice that DP values stayed pretty low across most species, no matter the functional group or phenology stage. This matches up with what Katey was seeing in Asotin Creek.
...
I also looked at whether the percentage of non-native species observed at our transects increased at any point during the season. Overall, the number of non-native species remained fairly consistent throughout the year, with a small spike in the spring. However, since much fewer transects were sampled in April compared to any other month, it’s unclear whether this pattern accurately reflects conditions across the landscape at that time. There also seems to be an uptick in invasive species when we sampled in the fall which makes sense because anectdotally are largest biomass seemed to be cheatgrass in most of our plots.

```{r}
status <- plant %>% 
  select(Spp, Status, FG_New) %>% 
  rename(FG = FG_New)
