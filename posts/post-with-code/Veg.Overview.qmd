...
**(NOTE FOR RYAN: I am not sure if a line graph like the one below or the stacked bar graph that I used above is a better representation of the data in this section.)**
...
comp %>%
  ...
  geom_point(size = 2) +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Combined Phenology Timing", y = "Proportion", x = "Month", color = "Phenology stage") +
  theme_minimal()
```
...
p2 <- ggplot(pvt_comp, aes(x = Month, y = prop, fill = Pheno,
  ...
  geom_col() +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Phenology Timing Across PVTs", y = "Proportion", x = "Month", fill = "Phenology stage") +
...
df %>%
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Status Composition based on PVT",
    x = "PVT",
    y = "Proportion",
    fill = "Status"
  ) +
...
-   I’m getting really close to finalizing my biomass predictions. Right now, I’m refining the linear regression models to make sure they’re accurately predicting usable biomass across transects using the FRESH model.

-   I’m still processing the quality samples from this summer, but I should have everything ground and submitted for analysis before Thanksgiving. Hopefully, we’ll have the 2025 quality results back around January. In the meantime, I can start building most of my models using the 2024 quality results and rerun them once the 2025 results are available.
...
-   Once I have both the suitable biomass estimates and my GAM, I can start building the HSF and iSSA to finally look at how the sheep are making space-use decisions based on the nutritional landscape.
