...
transect_area_m2 <- 5  # 5 quadrats of 1x1 m each

total <- plot.totals %>%
  mutate(SuitableBiomass_kg_ha = (SuitableBiomass / 1000) / transect_area_m2 * 10000)
```
...
ggplot(plot.totals, aes(x = Date, y = SuitableBiomass)) +
  ...
  theme_minimal() +
  labs(
    title = "Suitable Biomass (kg/ha) Over Time by PVT",
    x = "Date",
    y = "Suitable Biomass"
  )
```
...
ggplot(avg_sb, aes(x = factor(PVT), y = mean_sb)) +
  ...
  theme_minimal() +
  labs(
    title = "Average Suitable Biomass (kg/ha) by PVT",
    x = "PVT",
