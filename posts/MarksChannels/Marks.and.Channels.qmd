...
biomass1 <- comp1 %>%
  ...
  mutate(totalbiomass = mean(DryWeight.g., na.rm = TRUE)) %>%  
  ungroup() %>%
  slice_max(totalbiomass, n = 10) 
cover1 <- comp1 %>% 
  group_by(ActualName.x) %>%
  summarise(totalcover = mean(Percent, na.rm = TRUE)) %>%  
  ungroup() %>%
  slice_max(totalcover, n = 10)
...
#ggplot(cover1, aes(x = ActualName.x, y = totalcover, group = PVT, color = PVT)) +
#  geom_line(linewidth = 1.2) +
#  geom_point(size = 3) +  # Add points for clarity
#  labs(
#    title = "Total Percent Cover by Species",
#    x = "Species (ActualName.x)",
#    y = "Total Percent Cover"
#  ) +
#  scale_color_viridis_d(name = "Plot ID") +
#  theme_minimal() +
#  theme(
...
Cheatgrass is the most common species observed amount all vegetation
communities (should I do a different graph to figure this out?) I want
to see how its biomass changes overall throughout the course of my field
season.
...
cheat <- comp1 %>%
  filter(Spp == "BRTE") %>%  
  group_by(PlotID) %>%       
  mutate(TotalDryWeight = sum(DryWeight.g., na.rm = TRUE))  

cheat2 <- comp1 %>%
  filter(Spp == "BRTE") %>%  
  group_by(PlotID) %>%       
  mutate(TotalCover = sum(Percent, na.rm = TRUE))  

View(cheat)

ggplot(cheat, aes(x = Julian_Day.x, y = TotalDryWeight))+
         geom_point()+
         geom_smooth(method = "lm", se = FALSE, color = "black") +
         labs(Title = "Cheatgrass Biomass",
              x = "Julian Day",
              y = "Total Biomass (g)")+
         theme_minimal()

ggplot(cheat2, aes(x = Julian_Day.x, y = Percent))+
         geom_point()+
         geom_smooth(method = "lm", se = FALSE, color = "black") +
...
ggplot(totalbio, aes(x = Julian_Day.x, y = biomass, color = PVT)) +
  geom_point(alpha = 1) +  
  geom_smooth(method = "loess", span = 0.5, color = "blue", fill = "lightblue")+ 
  labs(title = "Biomass trends by Elevation",
       x = "Julian Day", y = "Biomass (g)", color = "PVT") +
  scale_color_manual(values = c("668" = "violet", "669" = "red", "674" = "yellow", "682"= "blue", "672" = "green"))+
  theme_gray()
```

```{r}
ggplot(comp1, aes(x = Percent, y = DryWeight.g., color = FunctionalGroup)) +
  geom_boxplot() +
  labs(title = "Relationship Between Percent Cover and Biomass",
       x = "Percent Cover (%)", y = "Biomass (g)") +
  theme_minimal()
